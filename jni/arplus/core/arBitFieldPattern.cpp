...
static void applyMaskSimple(IDPATTERN& nPattern) {
	nPattern ^= fullMask;
}

static void applyMaskBCH(IDPATTERN& nPattern) {
	nPattern ^= bchMask;
}
...
static void rotate90CW(IDPATTERN& nPattern) {
...
static float checkPatternBitSimple(IDPATTERN nPattern, int nBit, int& nValue) {
	int b0 = (int) (nPattern >> (posMask0 + nBit)) & 1, b1 = (int) (nPattern >> (posMask1 + nBit)) & 1, b2 =
			(int) (nPattern >> (posMask2 + nBit)) & 1, b3 = (int) (nPattern >> (posMask3 + nBit)) & 1, sum = b0 + b1
			+ b2 + b3;
...
static void checkPatternSimple(IDPATTERN nPattern, int& nID, float& nProp) {
	nProp = 0.0f;
	nID = 0;
...
static void checkPatternBCH(IDPATTERN nPattern, int& nID, float& nProp, BCH* nProcessor) {
	int err = -1;
...
int Tracker::downsamplePattern(uint8_t* data, unsigned char* imgPtr) {
	...
	if (PATTERN_WIDTH == 18 && PATTERN_HEIGHT == 18) {
		...
		/*
		 ...
		 fclose(fp);
		 */
	} else if (PATTERN_WIDTH == 12 && PATTERN_HEIGHT == 12) {
		...
		/*
		 ...
		 fclose(fp);
		 */
	} else if (PATTERN_WIDTH == 6 && PATTERN_HEIGHT == 6) {
...
int Tracker::bitfield_check_simple(uint8_t *data, int *code, int *dir, ARFloat *cf, int thresh) {
	...
	if (prop0 >= prop90 && prop0 >= prop180 && prop0 >= prop270) // is prop0 maximum?
	{
		...
		*cf = prop0;
		*code = id0;
	} else if (prop90 >= prop0 && prop90 >= prop180 && prop90 >= prop270) // is prop90 maximum?
	{
		*dir = 1;
		*cf = prop90;
		*code = id90;
	} else if (prop180 >= prop0 && prop180 >= prop90 && prop180 >= prop270) // is prop180 maximum?
	{
		*dir = 2;
		*cf = prop180;
		*code = id180;
	} else if (prop270 >= prop0 && prop270 >= prop90 && prop270 >= prop180) // is prop270 maximum?
	{
		*dir = 3;
		*cf = prop270;
		*code = id270;
