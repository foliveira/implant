...
int Tracker::arLoadPatt(char *filename) {
	...
	for (h = 0; h < 4; h++) {
		...
		for (i3 = 0; i3 < 3; i3++) {
			for (i2 = 0; i2 < PATTERN_HEIGHT; i2++) {
				for (i1 = 0; i1 < PATTERN_WIDTH; i1++) {
					...
					if (i3 == 0)
						patBW[patno][h][i2 * PATTERN_WIDTH + i1] = j;
					else
						patBW[patno][h][i2 * PATTERN_WIDTH + i1] += j;
					if (i3 == 2)
						patBW[patno][h][i2 * PATTERN_WIDTH + i1] /= 3;
					l += j;
				}
			}
		}
		...
		if (patpow[patno][h] == 0.0)
			patpow[patno][h] = (ARFloat) 0.0000001;
...
int Tracker::arGetPatt(uint8_t *image, int *x_coord, int *y_coord, int *vertex, uint8_t **pext_pat[3]) {
	...
	lx1 = (int) ((local[0][0] - local[1][0]) * (local[0][0] - local[1][0]) + (local[0][1] - local[1][1]) * (local[0][1]
			- local[1][1]));
	lx2 = (int) ((local[2][0] - local[3][0]) * (local[2][0] - local[3][0]) + (local[2][1] - local[3][1]) * (local[2][1]
			- local[3][1]));
	ly1 = (int) ((local[1][0] - local[2][0]) * (local[1][0] - local[2][0]) + (local[1][1] - local[2][1]) * (local[1][1]
			- local[2][1]));
	ly2 = (int) ((local[3][0] - local[0][0]) * (local[3][0] - local[0][0]) + (local[3][1] - local[0][1]) * (local[3][1]
			- local[0][1]));
	if (lx2 > lx1)
		lx1 = lx2;
	if (ly2 > ly1)
	...
	if (arImageProcMode == AR_IMAGE_PROC_IN_FULL) {
		...
		while (xdiv2 * xdiv2 * 4 < lx1 / 4)
			xdiv2 *= 2;
		while (ydiv2 * ydiv2 * 4 < ly1 / 4)
			ydiv2 *= 2;
	}
	if (xdiv2 > PATTERN_SAMPLE_NUM)
		xdiv2 = PATTERN_SAMPLE_NUM;
	if (ydiv2 > PATTERN_SAMPLE_NUM)
		ydiv2 = PATTERN_SAMPLE_NUM;
	...
	//
	{
	    ...
	    memset((void*)ext_pat2, 0, PATTERN_HEIGHT * PATTERN_WIDTH * 3 * sizeof(uint32_t));
